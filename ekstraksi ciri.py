...
for root, dirs, files in os.walk("Data Sensor/Data_Berbaring/"):
    for filename in files:
        namedfile.append(filename)

for j in range(len(namedfile)):

    dataset = pd.read_csv("Data Sensor/Data_Berbaring/" + str(namedfile[j]))
    ...
    def plot_signal_decomp(data, w, title):
        ...
        feature_extraction = [min_coefD[4], max_coefD[4], mean_coefD[4], stdev_coefD[4],median_coefD[4],var_coefD[4], 6]
...
print(all_feature)
np.savetxt("Data Training/Data_Berjalan_1.csv", all_feature, header="min,max,mean,stdev,median,variance,label", delimiter=",")
